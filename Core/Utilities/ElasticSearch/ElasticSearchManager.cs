...
namespace Core.Utilities.ElasticSearch
{
    public class ElasticSearchManager : IElasticSearch
    {
        ...
        public async Task<IResult> CreateNewIndexAsync(IndexModel indexModel)
        {
            ...
            var response = await elasticClient.Indices.CreateAsync(indexModel.IndexName, se => se.Settings(a => a.NumberOfReplicas(indexModel.NumberOfReplicas).NumberOfShards(indexModel.NumberOfShards)).Aliases(x => x.Alias(indexModel.AliasName)));
        ...
        public async Task<List<ElasticSearchGetModel<T>>> GetSearchByField<T>(SearchByFieldParameters fieldParameters) where T : class
        {
            ...
            .Query(q => q.Match(m => m.Field(fieldParameters.FieldName)
