...
{
    public class AgentAuthenticationProvider : AuthenticationProviderBase, IAuthenticationProvider
    {
        private readonly IUserRepository _userRepository;
        private readonly ITokenHelper _tokenHelper;
        public AuthenticationProviderType ProviderType { get; }

        public AgentAuthenticationProvider(AuthenticationProviderType providerType, IUserRepository userRepository, IMobileLoginRepository mobileLogin, ITokenHelper tokenHelper, ISmsService smsService)
            : base(mobileLogin,smsService)
        {
        ...
        public override async Task<LoginUserResult> Login(LoginUserCommand command)
        {
            ...
            if (command.IsPhoneValid)
                return await PrepareOneTimePassword(AuthenticationProviderType.Agent, user.MobilePhones, user.CitizenId.ToString());
            else
